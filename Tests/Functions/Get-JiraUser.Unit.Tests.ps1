...
Describe "Get-JiraUser" -Tag 'Unit' {
    ...
    Mock Invoke-JiraMethod -ModuleName JiraPS -ParameterFilter { $Uri -like "*/rest/api/*/user?accountId=*" } {
        Write-MockInfo 'Invoke-JiraMethod' @{Method = $Method; Uri = $Uri }
    ...
    Mock Invoke-JiraMethod -ModuleName JiraPS -ParameterFilter { $Uri -like "*/rest/api/*/user?username=*" } {
        Write-MockInfo 'Invoke-JiraMethod' @{Method = $Method; Uri = $Uri }
    ...
    Describe "Behavior checking" {
        It "gets the intormation about the logged in user" {
            ...
            Assert-MockCalled -CommandName Invoke-JiraMethod -ModuleName JiraPS -Exactly -Times 1 -Scope It -ParameterFilter { $URI -like "*/rest/api/*/myself" }
            Assert-MockCalled -CommandName Invoke-JiraMethod -ModuleName JiraPS -Exactly -Times 1 -Scope It -ParameterFilter { $URI -like "*/rest/api/*/user?accountId=*&expand=groups" }
        }
